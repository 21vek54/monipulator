...
void startConnect(const String &ssid, const String &password, bool machineBusy)
{
    ...
    if (machineBusy) {
        Serial.println("WiFi: РєРѕРјР°РЅРґР° РЅРµРґРѕСЃС‚СѓРїРЅР° РІРѕ РІСЂРµРјСЏ РґРІРёР¶РµРЅРёСЏ/С†РёРєР»Р°.");
        return;
    }
...
{
    if (machineBusy) {
        Serial.println("WiFi scan: РЅРµРґРѕСЃС‚СѓРїРЅРѕ РІРѕ РІСЂРµРјСЏ РґРІРёР¶РµРЅРёСЏ/С†РёРєР»Р°.");
        return;
    }
